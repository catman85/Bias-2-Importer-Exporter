  ...
  methods: {
    ...
    async showNewNamePrompt(obj) {
      ...
      prompt({
        ...
        .then((r) => {
          if (!r) return console.log("user cancelled");
          else {
    ...
    async changeName(newName, obj, type) {
      ...
      if (type === this.objType.PRESET) {
        let metaPath = this.nativePath(
          this.selectedBankPath + "/" + obj.preset_uuid + "/meta.json"
        );
    ...
    async importPreset(path, bank, importType) {
      ...
      let selBankPath = this.nativePath(
      ...
      let currBankQobj = await this.getJsonQObject(
        presetJsonPathSelBank,
        "utf-8"
      );
